...
def create_database():
    try:
        with sqlite3.connect("./databases/jobs.db") as conn:
            print(f"Opened SQLite database jobs with version {sqlite3.sqlite_version} successfully.\n")
...
def create_job(table, fields, values):
...
def create_jobs(table, fields_list, values_list):
    output = []
...
def read_jobs_field(table, field, value):
    try:
        with sqlite3.connect("./databases/jobs.db") as conn:
            cursor = conn.cursor()
...
def read_jobs_field(table, field, value_list):
    output = []
